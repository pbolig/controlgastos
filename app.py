...
def agregar_categoria():
    try:
        ...
        db = get_db()
        cursor = db.cursor(cursor_factory=RealDictCursor)
        cursor.execute("INSERT INTO categorias (nombre) VALUES (%s) RETURNING id", (nombre.strip(),))
        db.commit()
...
def agregar_transaccion():
    try:
        ...
        db.commit()
        return jsonify({ "mensaje": "Transacción agregada exitosamente", "id": cursor.fetchone()['id'] }), 201
    except Exception as e:
...
def agregar_recurrente():
    try:
        ...
        db.commit()
        return jsonify({ "mensaje": "Movimiento recurrente guardado", "id": cursor.fetchone()['id'] }), 201
    except Exception as e:
...
def eliminar_recurrente(id):
    try:
        ...
        with db:
            ...
            db.execute("DELETE FROM pagos_recurrentes_log WHERE recurrente_id=%s", (id,))
            # Luego borramos el recurrente en sí
            cursor = db.execute("DELETE FROM gastos_recurrentes WHERE id=%s", (id,))
...
def get_recurrentes_status():
    try:
        ...
        cursor.execute("""
            ...
            LEFT JOIN pagos_recurrentes_log AS l ON r.id = l.recurrente_id AND l.mes = %s AND l.anio = %s
...
def pagar_recurrente():
    try:
        ...
        cursor = db.cursor(cursor_factory=RealDictCursor)
        with db:
            cursor.execute("SELECT * FROM gastos_recurrentes WHERE id=%s", (datos['recurrente_id'],))
            ...
            cursor.execute("INSERT INTO transacciones (descripcion, monto, tipo, fecha, categoria_id, moneda, comprobante_path) VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id",
                           (desc, float(datos['monto_pagado']), tipo_movimiento, hoy, recurrente['categoria_id'], recurrente['moneda'], comprobante_path))
            
            new_id = cursor.fetchone()['id']
            cursor.execute("INSERT INTO pagos_recurrentes_log (recurrente_id, transaccion_id, mes, anio) VALUES (%s, %s, %s, %s)",
    ...
    try:
        ...
        for moneda in ['ARS', 'USD']:
            ...
            cursor.execute("""
                ...
                WHERE r.moneda=%s AND r.id NOT IN (SELECT recurrente_id FROM pagos_recurrentes_log WHERE mes=%s AND anio=%s)
...
def pagar_cuota():
    try:
        ...
        cursor = db.cursor(cursor_factory=RealDictCursor)
        with db:
            cursor.execute("SELECT * FROM planes_cuotas WHERE id=%s", (datos['plan_id'],))
...
def eliminar_cuota(id):
    try:
        db = get_db()
        cursor = db.cursor(cursor_factory=RealDictCursor)
        cursor.execute("DELETE FROM planes_cuotas WHERE id=%s", (id,))
        db.commit()
...
def pagar_resumen_tarjeta():
    try:
        ...
        with db:
            ...
            cursor.execute("SELECT * FROM gastos_recurrentes WHERE id=%s", (recurrente_id,))
            ...
            cursor.execute("""
                ...
                WHERE recurrente_id = %s AND moneda = %s AND cuota_actual < total_cuotas
                AND (ultimo_pago_anio IS NULL OR ultimo_pago_anio < %s OR (ultimo_pago_anio = %s AND ultimo_pago_mes < %s))
...
def webauthn_login_complete():
    ...
    db = get_db()
    cursor = db.cursor(cursor_factory=RealDictCursor)
    cursor.execute("SELECT * FROM webauthn_credentials WHERE credential_id = %s", (credential_id,))
    ...
    try:
        ...
        cursor.execute("UPDATE webauthn_credentials SET sign_count = %s WHERE id = %s", (verification.new_sign_count, credential_data['id']))
        db.commit()
